...
public class DiamondTopology {
    ...
    public static void main(String[] args) throws Exception {
        ...
        treemaps.add(new TreeMap<Double, Integer>());
        int numberOfBolts = 4;
        String path = args[2]+"diamond/";//Path to files
        String mode = args[1];
        ...
        if (mode.equals("opt")) {//Optimized-Equality
            ...
            builder.setBolt("MultiplierBolt1", new MultiplierBolt1(treemaps.get(1),treemaps.get(2), selectivities.get(0), mode), ex).directGrouping("TopologySpout").setNumTasks(ex);
            builder.setBolt("AddingBolt1", new AddingBolt1(treemaps.get(3), selectivities.get(1), mode), ex).directGrouping("MultiplierBolt1").setNumTasks(ex);
            builder.setBolt("AddingBolt2", new AddingBolt2(treemaps.get(3), selectivities.get(2), mode), ex).directGrouping("MultiplierBolt1").setNumTasks(ex);
            builder.setBolt("MultiplierBolt2", new MultiplierBolt2(), ex).directGrouping("AddingBolt1").directGrouping("AddingBolt2").setNumTasks(ex);
            ...
            builder.setBolt("AddingBolt1", new AddingBolt1(selectivities.get(1), mode)).shuffleGrouping("MultiplierBolt1");
            builder.setBolt("AddingBolt2", new AddingBolt2(selectivities.get(2),mode)).shuffleGrouping("MultiplierBolt1");
            builder.setBolt("MultiplierBolt2", new MultiplierBolt2()).shuffleGrouping("AddingBolt1").shuffleGrouping("AddingBolt2");
            Config config = new Config();
            config.setMessageTimeoutSecs(300);
            StormSubmitter.submitTopology(args[0], config, builder.createTopology());
